 ...
 * テスト対象:
 * - アプリケーションコンテキストの取得
 * - CalculatorApplicationクラスの正しい動作
 * - Androidフレームワークとの統合
 *
 * インストルメンテーションテストの特徴:
 * - 実際のAndroid環境で実行
...
class ExampleInstrumentedTest {
    ...
    fun useAppContext() {
        ...
        assertEquals("com.takuyafukumura.calculator", appContext.packageName)
    }

    /**
     * CalculatorApplicationクラスの型検証テスト
     *
     * アプリケーションクラスが正しくCalculatorApplicationとして
     * インスタンス化されていることを確認します。
     *
     * 検証内容:
     * - ApplicationクラスがCalculatorApplicationとして認識される
    ...
    fun myApplication_isCorrectType() {
        ...
        // アプリケーションクラスがCalculatorApplicationのインスタンスであることを確認
        assertTrue(
            "Application should be instance of CalculatorApplication",
            application is CalculatorApplication,
        )
    }
}
